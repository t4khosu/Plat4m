	...
	public void canMoveUpdate(){
		...
		if(input.typedJ || input.typedK){
			...
			if((i = player.getJK()[key]) != null){ /* holding an item */
				...
				//Hitting:
				if(player.getLevel().getType() != levelType.HOUSE && player.getLevel().getType() != levelType.VILLAGE){ /* level where can hit */
				...
				//Fishing:
				if(i.getType() == ItemType.POLE && player.getPSM().fishing.isCatching()){
					player.getPSM().fishing.increaseTypeCounter();
				}
				
				//eat berry
				
				if(i.getType() == ItemType.BERRY &&  !ptm.getPotionTimer().isActive()){
					player.eatBerry(i);
				}
			}
		}
